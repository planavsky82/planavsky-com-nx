...
class CollectionComponent extends HTMLElement {
    ...
    disconnectedCallback() {
        ...
        if (this._previousButton) {
            this._previousButton.removeEventListener('click', () => { return false; });
        }
        if (this._nextButton) {
            this._nextButton.removeEventListener('click', () => { return false; });
        }
        this._div.removeEventListener('scroll', () => { return false; });
    }
    adoptedCallback() {
    ...
    loadModal() {
        ...
        const detailModalBg = document.createElement('div');
        const detailModalHeader = document.createElement('div');
        const detailModalTitle = document.createElement(`h${Number(this.getAttribute('sectionHeader')) + 1}`);
        const detailModalClose = document.createElement('button');
    ...
    toggleModal(open, callback, size, index, triggerElement) {
        ...
        const dialogBg = this._shadow.querySelector('.detail-modal-bg');
        if (dialog) {
    ...
    loadModalContent(data, index) {
        const content = this._shadow.querySelector('dialog > div.detail-modal-content');
        if (!content) {
            return;
        }
        console.log(data);
        ...
        switch (data.modal.type) {
            case 'tables': {
                const title = document.createElement(`h${Number(this.getAttribute('sectionHeader')) + 2}`);
            ...
            case 'ranking': {
                ...
                button.onclick = () => {
                    ...
                    const currentValue = data.modal.data.ranking;
    ...
    setSiblingClasses(activeId, max) {
        ...
        const forward = (activeId < (max - 3)) ? activeId + 3 : max;
        const backward = (activeId >= 3) ? activeId - 3 : 0;
        //console.log('max', max);
    ...
    set items(value) {
        ...
        <div class="header-actions"></div></div>`;
            ...
            // print name section
            const printNameWrapper = document.createElement('div');
            ...
            if (item.actions) {
                item.actions.forEach((action, actionIndex) => {
                    ...
                    const actionItemAnchor = document.createElement('a');
                    ...
                    if (action.modal) {
                        actionItemAnchor.addEventListener('click', (clickEvent) => {
                            var _a;
                            this.toggleModal(true, action.event, (_a = action.modal) === null || _a === void 0 ? void 0 : _a.size, index, clickEvent.currentTarget);
                        });
                    }
                    else {
                    ...
                    (_a = itemElement.querySelector('.header-section .header-actions')) === null || _a === void 0 ? void 0 : _a.appendChild(topAction);
                    topAction.href = 'javascript:';
                    if (action.modal) {
                        topAction.addEventListener('click', () => {
                            var _a;
                            this.toggleModal(true, action.event, (_a = action.modal) === null || _a === void 0 ? void 0 : _a.size, index);
                        });
                    }
                    else {
            ...
            ctx === null || ctx === void 0 ? void 0 : ctx.lineTo(200, 100);
            ctx === null || ctx === void 0 ? void 0 : ctx.stroke();
            // control area
            const controlElement = document.createElement('div');
            const rankUpElement = document.createElement('button');
            const rankDownElement = document.createElement('button');
    ...
    move(direction, activeIndex, activeId) {
        const order = this._items.filter((item) => {
            return item.id !== activeId;
        });
        let newIndex = activeIndex;
        const activeObj = this._items.find((item) => {
            return item.id === activeId;
        });
        if (direction === 'down') {
            newIndex++;
        ...
        this.items = order;
        const eventOrderAdjusted = new CustomEvent('order-adjusted', {
            detail: {
        ...
        const button = this._shadow.querySelector('#item_' + newIndex + ' .control-area button:nth-of-type(' + buttonPosition + ')');
        if (button) {
            button.focus();
        }
